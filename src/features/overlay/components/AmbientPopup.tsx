...
// The ambient hotkey surface (the `orb` window). Transparent, always-on-top, dropped at the bottom
// of the active screen by ⌥+Space — a premium command bar to ask Piku a quick question over
// whatever app you're in. Answers run on the local Ollama model (fast + private); the chips prefill
// the bar. The full tool-using agent lives in the main OS window.
...
export function AmbientPopup() {
  ...
  // Send → answer locally (Ollama), streaming the reply in place. Self-contained: no tools, no
  // cloud — a fast, private quick-ask. think:false keeps it snappy (no visible reasoning here).
  const onSend = useCallback(async () => {
    const t = text.trim()
    if (!t || guard.current) return
    guard.current = true
    setYou(t); setText(''); setReply(''); setBusy(true)
    try {
      const sys = `${PIKU_PERSONA}\n\nYou're answering from an ambient quick-command bar — be brief, direct, and useful.`
      let acc = ''
      await ollamaService.chatStream(
        [{ role: 'system', content: sys }, { role: 'user', content: t }],
        (c) => { acc += c; setReply(acc) },
        undefined, 0.7, 240_000, false,
      )
      if (!acc.trim()) setReply('(no response — is Ollama running? `ollama serve`)')
    } catch {
      setReply("I can't reach Ollama right now — start it with `ollama serve`.")
    } finally {
      ...
      <div className="w-full max-w-2xl flex flex-col gap-2">
        {(you || reply || busy) && (
          <div className="self-stretch rounded-2xl bg-gradient-to-b from-white/[0.06] to-white/[0.015] backdrop-blur-2xl border border-white/10 px-4 py-2.5 shadow-[0_16px_60px_-12px_rgba(0,0,0,0.85)]">
            {you && <div className="text-[11px] text-white/35 mb-1">you · {you}</div>}
            <div className="text-sm text-white/80 whitespace-pre-wrap leading-relaxed">
              {reply
                ? <>Piku: {reply}{busy && <span className="animate-pulse text-cyan-300 ml-0.5">▌</span>}</>
                : busy ? <span className="text-white/45 italic">thinking…</span> : null}
            </div>
          </div>
        )}
        ...
        <div className="relative flex items-center gap-3 rounded-3xl bg-gradient-to-b from-white/[0.09] to-white/[0.03] backdrop-blur-2xl border border-white/12 ring-1 ring-inset ring-white/[0.05] pl-4 pr-3 py-2.5 shadow-[0_18px_70px_-14px_rgba(0,0,0,0.92),0_0_50px_-18px_rgba(34,211,238,0.55)]">
          ...
          <span className="relative flex shrink-0 w-9 h-9 items-center justify-center">
            <span className={`absolute w-6 h-6 rounded-full bg-cyan-400 opacity-15 blur-md ${busy ? 'animate-pulse' : ''}`} />
            <span className="w-3 h-3 rounded-full bg-cyan-400 shadow-[0_0_16px_3px_rgba(34,211,238,0.55)]" />
          </span>
          <div className="flex-1">
            <ChatInput value={text} isLoading={busy} onChange={setText} onSend={onSend} />
          </div>
